...
router.get('/dashboard/trend', requireAuth, async (req, res) => {
  const { userId, role } = req.auth!
  const where = role === 'CHILD' ? { userId } : {}
  const items = await prisma.$queryRaw<Array<{ month: string; total: number }>>`
...
router.post('/expenses', requireAuth, async (req, res) => {
  ...
  if (role === 'CHILD' && ownerId !== userId) return res.status(403).json({ error: 'Forbidden' })
...
router.put('/expenses/:id', requireAuth, async (req, res) => {
  ...
  if (role === 'CHILD' && existing.userId !== userId) return res.status(403).json({ error: 'Forbidden' })
